...
def main():
    ...
    parser.add_argument('--debug', dest="debug", help="if debugging")
    parser.add_argument('--dv', dest="debug_verbose", help="Show more info when debuging")
    ...
    parser.set_defaults(debug_verbose=False)
...
def regex_check(printableDiff, commit_time, branch_name, prev_commit, blob, commitHash, custom_regexes={}):
    ...
    for key in secret_regexes:
        if DEBUG and debug_verbose:
            print("RegCheck for '"+key+"'")
        found_strings = secret_regexes[key].findall(printableDiff)
        for found_string in found_strings:
            found_diff = printableDiff.replace(printableDiff, bcolors.WARNING + (' '.join(found_string) if (type(found_string) == tuple) else found_string) + bcolors.ENDC)
        if found_strings:
            foundRegex = {}
...
def find_strings(git_url, since_commit=None, max_depth=1000000, printJson=False, do_regex=False, do_entropy=True, surpress_output=True,
    ...
    output = {"foundIssues": []}
    if DEBUG:
            print("[Entropy: "+str(do_entropy)+"| Regex: "+str(do_regex)+"| Surpress: "+str(surpress_output)+"]")
    if repo_path:
    ...
    for remote_branch in branches:
        ...
        prev_commit = None
        if DEBUG:
            print("Checking Branch(Depth:"+str(max_depth)+"):"+branch_name)
        for curr_commit in repo.iter_commits(branch_name, max_count=max_depth):
            if DEBUG and debug_verbose:
                print("Commit:"+curr_commit.summary+"["+str(datetime.datetime.fromtimestamp(curr_commit.committed_date).strftime('%Y-%m-%d %H:%M:%S'))+"]")
            ...
            # if DEBUG and len(foundIssues)>0:
            #     print("Issue:"+str(len(foundIssues)))
